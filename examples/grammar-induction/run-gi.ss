...
(define (opt-select xs idx val)
  (if (< (length xs) (+ idx 1)) val (list-ref xs idx)))

(define argv (command-line))
(define file-to-load (list-ref argv 1))
(define fan-out (string->number (opt-select argv 3 "10")))
(define num-iter (string->number (opt-select argv 4 "100")))
...
(define best-gr (car (grammar->grammar+posterior best-state)))

(print "Grammar:")

(pretty-print best-gr)

(print "Sample:")

(pretty-print `(
                (define sample ,(sample-grammar+parameters best-gr))
