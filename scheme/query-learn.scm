(library (query-learn)
         ...
         (define noisy-arith-recursion-dearguments 
           (make-dearguments-transformation predicate-recursion-replacement-noisy))

         (define arith-deargument-transform (mk-syntactic-deargument-transform
                                              learn-substitutions))

         (define noisy-arith-deargument-transform (mk-syntactic-deargument-transform
           ...
           (lambda (program . nofilter)
             (let* ([abstrs-with-vars (filter has-arguments? (program->abstractions program))]
                    [transformed-programs 
                      (delete '() (map (curry transform-fx program) 
                                       abstrs-with-vars))] 
         ...
         (define (mk-syntactic-deargument-transform sub-fx)
           ...
           (lambda (prog abstr)
             (let* (
                    [substitutions (sub-fx prog abstr)]
