...
(define (process-xml port)
  (let* ([db (pp "hi")]
         [doc (ssax:xml->sxml port '())]
         [arrays-only ([sxpath '(plist dict array)] doc)]
         )
    arrays-only))
...
(define (main argv)
  ...
  (let* ([db (pp "hi2")]
         [processed-data (call-with-input-file (cadr argv) (lambda (port) (process-xml port)))]
         [db (pp processed-data)])
    (with-output-to-file (caddr argv)
                         (lambda () 
                           (pp processed-data)) 'replace)))
